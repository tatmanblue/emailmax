...
type
  TwndFolder = class(TForm)
    ...
    procedure ctlMessagesKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure ctlMessagesStartDrag(Sender: TObject;
      var DragObject: TDragObject);
    procedure ctlMessagesEndDrag(Sender, Target: TObject; X, Y: Integer);
  ...
  published
    property FolderId : Integer read m_nFolderId write m_nFolderId;
  end;
...
begin
   ...
   g_oLogFile.Write('Getting oItem for Delete');
   oItem := ctlMessages.ItemFocused;
   if NIL = oItem then
   begin
       MessageDlg('Please select a message to delete.', mtInformation, [mbOK], 0);
   ...
   if NIL = g_oFolders[m_nFolderId] then
   begin
   ...
   if NIL = g_oFolders[g_cnTrashFolder] then
   begin
   ...
   // question is: why?  its not consistant
...
begin
   ...
   assert(wndNotepad <> NIL, 'wndNotepad is Nil in TwndFolder.OpenSelectedMsg');
...
var
   nWindowType: TWindowFocused;
begin
   case m_nFolderId of
       g_cnInFolder: nWindowType := wfInbound;
       g_cnOutFolder: nWindowType := wfComplete;
       g_cnToSendFolder: nWindowType := wfOutbound;
       g_cnTrashFolder: nWindowType := wfTrash;
       else exit;
   end;

   wndMaxMain.ChildWndReceivedFocus(nWindowType);
   Invalidate;
   Update;
end;
...
procedure TwndFolder.ctlMessagesStartDrag(Sender: TObject;
  var DragObject: TDragObject);
begin
   ctlStatus.SimpleText := 'Moving a message...';
end;

procedure TwndFolder.ctlMessagesEndDrag(Sender, Target: TObject; X,
  Y: Integer);
begin
   ctlStatus.SimpleText := '';
end;
