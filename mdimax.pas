...
type
  ...
  TwndMaxMain = class(TForm)
    ...
    procedure mnViewContactsClick(Sender: TObject);
  private
    ...
    procedure SetupContactToolbarAndMenu;
  ...
  public
    ...
    procedure HandleMDIChildCloses(var Msg : TMessage); Message WM_HANDLE_MDICHILD_CLOSES;
...
uses
   ...
   eregistry, helpengine, wndNewMailDisplay, wndPrintPreview, wndContactList;
...
procedure TwndMaxMain.HandleMDIChildCloses(var Msg : TMessage);
var
   eFocus : TWindowFocused;
begin

   eFocus := TWindowFocused(Msg.WParam);

   case eFocus of
       wfOutbound: wndOutbound := NIL;
       wfInbound: wndInbound := NIL;
       wfComplete: wndComplete := NIL;
       wfTrash: wndTrash := NIL;
       wfDraft: wndDraft := NIL;
       wfContacts: wndContacts := NIL;
   ...
   begin
       ...
       begin
           ...
           case nWndType of
               ...
               wfContacts: SetupContactToolbarAndMenu;
               // TODO...
               wfUser:
               begin
                   Exit;
...
procedure TwndMaxMain.SetupContactToolbarAndMenu;
begin
   SetControlStates(pbCheckMail, TRUE);
   SetControlStates(pbOpenEmail, TRUE);
   SetControlStates(pbSendNow, TRUE);
   {
   // TODO: 08.06.02 Implement Printing, Exporting contacts
   SetControlStates(pbPrint, TRUE);
   SetControlStates(mnFilePrint, TRUE);
   SetControlStates(mnFilePreview, TRUE);
   SetControlStates(mnFileSaveAs, TRUE);
...
begin
   ...
   for nCount := MDIChildCount - 1 downto 0 do
      MDIChildren[nCount].Close;
...
procedure TwndMaxMain.mnViewContactsClick(Sender: TObject);
begin

   if Not Assigned(wndContacts) then
       wndContacts := TwndContactList.Create(Self);

   if FALSE = wndContacts.Visible THEN
       wndContacts.Visible := TRUE;

   RestoreChildWnd(wndContacts);
   ChildWndReceivedFocus(wfContacts);
   wndContacts.Show;
...
begin
   ...
   if Not Assigned(wndTrash) then
       raise Exception.Create('wndTrash not assigned in TwndMaxMain.mnViewTrashClick');
